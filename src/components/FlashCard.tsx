...
export default function FlashCard({ section, term, definition }: FlashCardProps) {
  ...
  return (
    <button
      ...
      aria-label={
        lang === 'es'
          ? `Tarjeta: ${term}. Haz clic para ${flipped ? 'mostrar el término' : 'revelar la definición'}`
          : `Flashcard: ${term}. Click to ${flipped ? 'show term' : 'reveal definition'}`
      }
    >
      {!flipped ? (
        <div className="flex flex-col justify-between h-full min-h-[112px]">
          ...
          <p className="text-[11px] text-navy-800/30 mt-4">{lang === 'es' ? 'Toca para revelar la definición' : 'Tap to reveal definition'}</p>
        </div>
      ) : (
        <div className="flex flex-col justify-between h-full min-h-[112px]">
          <div>
            <p className="text-[10px] font-bold tracking-[0.1em] uppercase text-navy-700/35 mb-3">{lang === 'es' ? 'Definición' : 'Definition'}</p>
            <p className="text-[13px] text-navy-800/70 leading-relaxed">{definition}</p>
          </div>
          <p className="text-[11px] text-navy-800/30 mt-4">{lang === 'es' ? 'Toca para ver el término' : 'Tap to see term'}</p>
        </div>
      )}
    </button>
  )
