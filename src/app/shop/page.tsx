...
import {
  getAllCollections, formatPrice, getFirstImage, getDefaultVariant,
  hasDiscount, ShopifyProduct,
} from '@/lib/shopify'
...
function ProductCard({ product }: { product: ShopifyProduct }) {
  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden border border-stone-100 hover:shadow-xl hover:shadow-stone-200/50 hover:-translate-y-0.5 transition-all duration-200 flex flex-col h-full">
      {/* Image */}
      <Link href={`/products/${product.handle}`} className="block relative overflow-hidden bg-stone-50 aspect-[4/3]">
        {image ? (
          <Image
            src={image.url}
        ...
        {discount && (
          <span className="absolute top-3 left-3 text-[10px] font-bold px-2 py-1 rounded-full text-white" style={{ backgroundColor: '#2EBB50' }}>
            Sale
          </span>
        )}
        {product.tags.includes('bestseller') && (
          <span className="absolute top-3 right-3 text-[10px] font-bold px-2 py-1 rounded-full text-white" style={{ backgroundColor: '#5BC4F8' }}>
            Bestseller
          </span>
        )}
      </Link>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <Link href={`/products/${product.handle}`} className="group/title flex-1">
          ...
          {product.description && (
            <p className="text-[12.5px] text-navy-800/50 leading-relaxed line-clamp-2">
              {product.description}
            </p>
          )}
        </Link>
...
export default async function ShopPage() {
  ...
  const isConfigured = !!process.env.NEXT_PUBLIC_SHOPIFY_STOREFRONT_ACCESS_TOKEN &&
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* ── Setup notice (shown only if token not configured) ── */}
      {!isConfigured && (
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-4">
          <div className="max-w-7xl mx-auto flex items-start gap-3">
            ...
            <p className="text-[13px] text-amber-800 leading-relaxed">
              <strong>Shopify not connected yet.</strong> Add your <code className="bg-amber-100 px-1 rounded text-[12px]">NEXT_PUBLIC_SHOPIFY_STOREFRONT_ACCESS_TOKEN</code> to <code className="bg-amber-100 px-1 rounded text-[12px]">.env.local</code> to load real products from your store.{' '}
              See the setup instructions in <code className="bg-amber-100 px-1 rounded text-[12px]">.env.local</code>.
            </p>
          </div>
        </div>
      )}
      ...
      {collections.length > 0 ? (
        ...
        /* Empty state when no collections exist yet */
        <section className="py-24 lg:py-32">
          <div className="max-w-7xl mx-auto px-6 lg:px-12 text-center">
            ...
            <h2 className="text-[1.4rem] font-bold text-navy-900 mb-3">Products coming soon</h2>
            <p className="text-[14px] text-navy-800/40 max-w-md mx-auto">
              Connect your Shopify store and add products to the <strong>families</strong>, <strong>professionals</strong>, and <strong>downloads</strong> collections to display them here.
            </p>
          </div>
        </section>
      )}
